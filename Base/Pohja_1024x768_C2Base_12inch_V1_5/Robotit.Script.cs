namespace Neo.ApplicationFramework.Generated
{
	...
	public partial class Robotit
	{
		...
		/// Robottien yhteydet.
		/// </summary>
		readonly Dictionary<int, Robotti> robotit = new Dictionary<int, Robotti>(); // Robotit
		/// <summary>
		/// Ikuisesti jatkuva robottiyhteyksien taustamonitoroinnin ajastin.
		/// </summary>
		private Timer Watchdog;
		/// <summary>
		/// robottien välinen 'kaiku', kaiuttaa viestit robottien välilä.
		/// </summary>
		//Lavaus.Echo echo;
		...
		void Robotit_Created(System.Object sender, System.EventArgs e)
		{
			...
			//echo = new Lavaus.Echo(1, 2); // avaa portit 50011 ja 50012 viesten välitystä varten
			...
			Watchdog = new Timer((args) =>
			{
				...
				foreach (int robotti in Globals._Konfiguraatio.CurrentConfig.Robots.Keys)
				{
					if (TarkistaTila(robotti)) Globals.Tags.HMI_CommFault_Robots.SetTag();
		...
		private bool TarkistaTila(int numero)
		{
			//lock (lockme)
				if (Exists(numero))
				{
			...
			return robotit[numero].WatchdogHalytys;
		}
		
		/// <summary>
		/// Palauttaa true, jos robotin lähetyspuskurissa on aloitussanomia
		/// </summary>
		/// <param name="rno"></param>
		/// <returns></returns>
		public bool AloitusSanomiaJonossa(int rno)
		{
			if (Exists(rno))
				return robotit[rno].OnkoJonossaAloitusSanomia();
			else
				return false;
		}
		...
		/// <summary>
		/// Ryhmittelyn lavanvaihto bobotille
		/// </summary>
		/// <param name="robotti"></param>
		/// <param name="lavapaikka"></param>
		/// <returns></returns>
		...
		public string TeeAloitus(int robotti, List<int> tuloradat, List<int> lavapaikat, int pattern, Lavaus.Kuvio kuvio)
		{
			...
			try
			{
				Globals.Tags.Log("TeeAloitus! " + robotti.ToString());
				Exists(robotti); // jos robotti ei ole olemassa, se luodaan

				ret = robotit[robotti].TeeAloitus(tuloradat, lavapaikat, pattern, kuvio, true);
			}
			catch (Exception ex)
			{
		...
		public void VaihdaKieli(int lang)
		{
			try
			{
				if (Globals.Tags.TraceAll) System.Diagnostics.Trace.WriteLine(string.Format("Change language to Robots [{0}]", lang));

				foreach (int robotti in robotit.Keys)
				{
					Exists(robotti); // jos robotti ei ole olemassa, se luodaan

					robotit[robotti].Kielenvaihto(lang);
				}
			}
			catch (Exception ex)
			{
