namespace Neo.ApplicationFramework.Generated
{
	...
	public partial class ProductRegister
	{
		...
		void ProductRegister_Opened(System.Object sender, System.EventArgs e)
		{
			...
			Globals.Tags.HMI_ProdReg_PalletPattern.ValueChange += LataaKuvio;

			foreach (int tyyppi in Globals._Konfiguraatio.CurrentConfig.Lavatyypit.Keys)
			{
		...
		void Btn_Valikkeet_Click(System.Object sender, System.EventArgs e)
		{
			...
			Globals.Tags.HMI_ProdCtrl_Cardboards_Pahvit.Value = Globals.Tags.HMI_ProdReg_Spacers.Value;
			...
			Globals.Tags.HMI_ProdCtrl_Cardboards_Kerrosasetus.Value = Globals.Tags.HMI_ProdReg_LayerCount.Value;
		...
		void Product_Save_As_btn_Click(System.Object sender, System.EventArgs e)
		{
			...
			if (Globals.Tags.HMI_ProdReg_RecipeSelected.Value.ToString().Length > 0)
			{
				...
				{
					Globals.Tags.HMI_Error_TextValue.SetAnalog((int)Neo.ApplicationFramework.Generated.ErrorTexts.ChoosePalletType);
		...
		void ValikeSivu_Closed(object sender, EventArgs e)
		{
			...
			Globals.Tags.HMI_ProdReg_Spacers.Value = Globals.Tags.HMI_ProdCtrl_Cardboards_Pahvit.Value;
		...
		void UpdatePatternPallettypes___(PatternInfo pi, int patternno)
		{
			if (patternno == 0)
			{
				foreach (int tyyppi in Globals._Konfiguraatio.CurrentConfig.Lavatyypit.Keys)
			...
			else
			{
				...
				foreach (int tyyppi in pi.Lavatyypit)
				{
					...
					if (first)
					{
						int pt = (int)Globals.Tags.HMI_ProdReg_PalletType.Value;
		...
		void LoadPicture(string fname)
		{
			...
			try
			{
				...
				if (!string.IsNullOrEmpty(fname))
				{
					...
					System.Diagnostics.Trace.WriteLine("Picture filename [" + name + "]");
			...
			catch (Exception ex)
			{
				...
				Globals.Tags.HMI_Error_TextValue.SetAnalog((int)Neo.ApplicationFramework.Generated.ErrorTexts.PatternImageLoadFailed);
		...
		void LataaKuvio(System.Object sender, System.EventArgs e)
		{
			...
			int patternno = Globals.Tags.HMI_ProdReg_PalletPattern.Value.Int;
			...
			{
				// tuntematon kuvio
				Globals.Tags.HMI_Error_TextValue.SetAnalog((int)Neo.ApplicationFramework.Generated.ErrorTexts.NotAllowedPattern);
			...
			//UpdatePatternPallettypes(pi, patternno);
			...
			try
			{
				...
				if (kuvio.Nykyinen != null)
				{
					...
					if (string.IsNullOrEmpty(fname)) fname = string.Format("{0}.jpg", patternno);					
					LoadPicture(fname);
				}
			}
			catch (Exception ex)
